...
export default function SendMoneyScreen() {
  ...
  const loadData = async () => {
    try {
      const [wallet, friendsList] = await Promise.all([WalletService.getWallet(), FriendService.getFriends()]);
  ...
  const confirmSend = async () => {
    ...
    try {
      ...
      router.back();
    } catch (error: any) { showError(error.response?.data?.message || 'Erreur envoi'); }
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { backgroundColor: COLORS.primary }]}>
          <TouchableOpacity onPress={() => setStep('form')} style={styles.backButton}><Text style={styles.backText}>←</Text></TouchableOpacity>
        ...
        <View style={styles.confirmCard}>
          ...
          <View style={styles.confirmDetails}>
            <View style={styles.confirmRow}><Text style={styles.confirmLabel}>Destinataire</Text><Text style={styles.confirmValue}>{receiverName}</Text></View>
            <View style={styles.confirmRow}><Text style={styles.confirmLabel}>Montant</Text><Text style={[styles.confirmValue, styles.amountValue]}>{formatAmount(parseFloat(amount))} Ar</Text></View>
            {description ? <View style={styles.confirmRow}><Text style={styles.confirmLabel}>Description</Text><Text style={styles.confirmValue}>{description}</Text></View> : null}
            <View style={styles.confirmDivider} />
            <View style={styles.confirmRow}><Text style={styles.confirmLabel}>Solde après</Text><Text style={styles.confirmValue}>{formatAmount(balance - parseFloat(amount))} Ar</Text></View>
          </View>
          <View style={styles.confirmActions}>
            <TouchableOpacity style={[styles.confirmButton, styles.cancelButton]} onPress={() => setStep('form')}><Text style={styles.cancelButtonText}>Retour</Text></TouchableOpacity>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: COLORS.primary }]}>
        ...
        <Text style={styles.headerTitle}>Envoyer de l'argent</Text><View style={{ width: 40 }} />
      </View>
      <View style={styles.balanceCard}><Text style={styles.balanceLabel}>Solde disponible</Text><Text style={styles.balanceAmount}>{formatAmount(balance)} Ar</Text></View>
      <View style={styles.formCard}>
        ...
        <View style={styles.amountContainer}>
          ...
          <TextInput style={styles.amountInput} value={amount} onChangeText={setAmount} keyboardType="numeric" placeholder="0" />
          <View style={styles.quickAmounts}>
            {amountPresets.map(preset => (
              <TouchableOpacity key={preset} style={[styles.quickAmountButton, parseFloat(amount) === preset && styles.quickAmountActive]} onPress={() => setAmount(preset.toString())}>
                <Text style={[styles.quickAmountText, parseFloat(amount) === preset && styles.quickAmountTextActive]}>{formatAmount(preset)}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        <View style={styles.descriptionContainer}>
          ...
          <TextInput style={styles.descriptionInput} value={description} onChangeText={setDescription} placeholder="Ex: Remboursement..." multiline />
        </View>
        <TouchableOpacity style={[styles.sendButton, (!receiverId || !amount || parseFloat(amount) > balance) && styles.sendButtonDisabled]} onPress={handleSend} disabled={!receiverId || !amount || parseFloat(amount) > balance}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setShowFriendPicker(false)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity>
            </View>
            <ScrollView>
              {friends.map(friend => (
                <TouchableOpacity key={friend.id} style={styles.friendItem} onPress={() => selectFriend(friend)}>
                  <View style={styles.friendAvatar}><Text style={styles.friendAvatarText}>{getInitials(friend.friend.firstName, friend.friend.lastName)}</Text></View>
                  <View><Text style={[styles.friendName, { color: colors.text }]}>{friend.friend.firstName} {friend.friend.lastName}</Text><Text style={styles.friendEmail}>{friend.friend.email}</Text></View>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  balanceCard: { backgroundColor: COLORS.white, marginHorizontal: 20, marginTop: 20, borderRadius: 20, padding: 20, alignItems: 'center', shadowColor: COLORS.black, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 5 },
  ...
  friendSelector: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: COLORS.gray300, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, marginBottom: 20 },
  ...
  amountInput: { borderWidth: 1, borderColor: COLORS.gray300, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, fontSize: 24, fontWeight: 'bold', color: COLORS.primary, textAlign: 'center' },
  ...
  descriptionInput: { borderWidth: 1, borderColor: COLORS.gray300, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, fontSize: 14, minHeight: 80, textAlignVertical: 'top' },

...
export default function AdminTransactionsScreen() {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => { const data = await AdminService.getAllTransactions(); setTxns(data.sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())); };
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, { backgroundColor: colors.card }]}>
            ...
            {item.sender && <Text>Expéditeur : {item.sender.firstName} {item.sender.lastName}</Text>}
            {item.receiver && <Text>Destinataire : {item.receiver.firstName} {item.receiver.lastName}</Text>}
          </View>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  card: { marginHorizontal: 20, marginBottom: 12, padding: 16, borderRadius: 12 },

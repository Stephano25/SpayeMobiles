  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: COLORS.primary }]}>
        ...
        <TouchableOpacity onPress={() => setEditMode(!editMode)}><Text style={styles.editButton}>{editMode ? 'Annuler' : '✏️'}</Text></TouchableOpacity>
      </View>
      <View style={styles.avatarContainer}><Text style={styles.avatarText}>{getInitials(user?.firstName || '', user?.lastName || '')}</Text></View>
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        {editMode ? (
          <>
            <TextInput style={styles.input} value={form.firstName} onChangeText={t => setForm({...form, firstName: t})} placeholder="Prénom" />
            <TextInput style={styles.input} value={form.lastName} onChangeText={t => setForm({...form, lastName: t})} placeholder="Nom" />
            ...
            <TextInput style={styles.input} value={form.phoneNumber} onChangeText={t => setForm({...form, phoneNumber: t})} placeholder="Téléphone" />
            <TouchableOpacity style={styles.saveButton} onPress={save} disabled={loading}><Text style={styles.saveButtonText}>Enregistrer</Text></TouchableOpacity>
          </>
        ) : (
          <>
...
const styles = StyleSheet.create({
  ...
  avatarContainer: { alignSelf: 'center', width: 100, height: 100, borderRadius: 50, backgroundColor: COLORS.primary, justifyContent: 'center', alignItems: 'center', marginTop: -30, marginBottom: 20 },
  ...
  input: { borderWidth: 1, borderColor: COLORS.gray300, borderRadius: 12, padding: 12, marginBottom: 12, fontSize: 16 },
  saveButton: { backgroundColor: COLORS.primary, padding: 14, borderRadius: 12, alignItems: 'center' },
  saveButtonText: { color: 'white', fontWeight: 'bold' },
  logoutButton: { backgroundColor: COLORS.error, marginHorizontal: 20, padding: 14, borderRadius: 12, alignItems: 'center', marginBottom: 40 },
